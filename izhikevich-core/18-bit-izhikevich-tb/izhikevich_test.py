...
async def test(dut):
    ...
    for current_step in range(timesteps):
        ...
        dut._log.info(f'''
            {fixed_point_to_decimal(str(dut.a.value), 10, 8)},
            {fixed_point_to_decimal(str(dut.b.value), 10, 8)},
            {fixed_point_to_decimal(str(dut.voltage.value), 10, 8)},
            {fixed_point_to_decimal(str(dut.w.value), 10, 8)},
            {fixed_point_to_decimal(str(dut.dw.value), 10, 8)},
            {fixed_point_to_decimal(str(dut.dw_step.value), 10, 8)},
        ''')
